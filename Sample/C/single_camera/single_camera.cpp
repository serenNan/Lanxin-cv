...
static int tof_depth_type =
    LX_DATA_TYPE::LX_DATA_UNSIGNED_SHORT; // 深度数据类型，默认为16位无符号整数
static int tof_amp_type =
    LX_DATA_TYPE::LX_DATA_UNSIGNED_SHORT; // 强度数据类型，默认为16位无符号整数
...
int main(int argc, char **argv)
{
    ...
    LX_STATE lx_state =
        DcOpenDevice((LX_OPEN_MODE)open_mode, open_param.c_str(), &handle, &device_info);
    if (LX_SUCCESS != lx_state)
    {
        std::cout << "open device failed, open_mode:" << open_mode << " open_param:" << open_param
    ...
    std::cout << "test_depth:" << test_depth << " test_amp:" << test_amp << " test_rgb:" << test_rgb
              << std::endl;
    ...
    while (true)
    {
        ...
        if ((LX_SUCCESS != ret) && (LX_E_FRAME_ID_NOT_MATCH != ret) // 帧ID不匹配（正常情况）
...
int TestDepth(bool is_enable, DcHandle handle)
{
    ...
    if (is_show)
    {
        ...
        cout << "depth_show shape:" << depth_show.type() << " " << depth_show.channels() << " "
             << depth_show.size() << endl;
...
int TestRgb(bool is_enable, DcHandle handle)
{
    ...
    // 创建OpenCV Mat对象用于显示
    cv::Mat rgb_show =
        cv::Mat(rgb_height, rgb_width, CV_MAKETYPE(rgb_data_type, rgb_channles), data_ptr);
